...
module.exports = function hrms(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/${filename}`;
      const newPath = `/uploads/hrms/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/hrms/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/${ filename }`
    });
  });
  form.parse(req);
};
