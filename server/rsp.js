...
module.exports = function rsp(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/rsp/${folder}/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/rsp/${folder}/${filename}`;
      const newPath = `/uploads/hrms/rsp/${folder}/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream(`/uploads/hrms/rsp/${folder}/${filename}`));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/rsp/${folder}/${filename}`
    });
  });
  form.parse(req);
};
