...
module.exports = function sourcecode(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/sourcecode/${filename}`)) {
      ...
      const oldPath = `/uploads/sourcecode/${filename}`;
      const newPath = `/uploads/sourcecode/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/sourcecode/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/sourcecode/${ filename }`
    });
  });
  form.parse(req);
};
