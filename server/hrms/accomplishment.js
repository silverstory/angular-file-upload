...
module.exports = function accomplishment(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/accomplishment/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/accomplishment/${filename}`;
      const newPath = `/uploads/hrms/accomplishment/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/hrms/accomplishment/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/accomplishment/${ filename }`
    });
  });
  form.parse(req);
};
