...
module.exports = function photos(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/photos/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/photos/${filename}`;
      const newPath = `/uploads/hrms/photos/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/hrms/photos/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/photos/${ filename }`
    });
  });
  form.parse(req);
};
