...
module.exports = function application(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/application/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/application/${filename}`;
      const newPath = `/uploads/hrms/application/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/hrms/application/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/application/${ filename }`
    });
  });
  form.parse(req);
};
