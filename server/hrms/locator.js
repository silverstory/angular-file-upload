...
module.exports = function locator(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/locator/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/locator/${filename}`;
      const newPath = `/uploads/hrms/locator/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/hrms/locator/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/locator/${ filename }`
    });
  });
  form.parse(req);
};
