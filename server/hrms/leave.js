...
module.exports = function leave(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/leave/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/leave/${filename}`;
      const newPath = `/uploads/hrms/leave/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/hrms/leave/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/leave/${ filename }`
    });
  });
  form.parse(req);
};
