...
module.exports = function rme(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/rme/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/rme/${filename}`;
      const newPath = `/uploads/hrms/rme/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/hrms/rme/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/rme/${ filename }`
    });
  });
  form.parse(req);
};
