...
module.exports = function learnings(req, res) {
  ...
  form.on('file', async (field, file) => {
    ...
    try {
    if (fs.existsSync(`/uploads/hrms/learnings/${filename}`)) {
      ...
      const oldPath = `/uploads/hrms/learnings/${filename}`;
      const newPath = `/uploads/hrms/learnings/${x}-v${v}.${filename.split('.').pop()}`;
    ...
    const readStream = fs.createReadStream(file.path)
      .pipe(fs.createWriteStream('/uploads/hrms/learnings/' + filename));
  });
  form.on('end', () => {
    res.json({
      path: `http://${ process.env.HOST }/hrms/learnings/${ filename }`
    });
  });
  form.parse(req);
};
